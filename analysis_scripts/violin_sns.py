...
#   data2_q = list(data2.quantile([0,0.25,0.5,0.75]))
    ...
    for i in range(2):
        for j in range(3):
            g1 = sns.violinplot(x="time_type", y="time_val", hue="image",data=data_CLI[data_CLI.time_type == timings[i*3+j]], ax=ax_CLI[i][j])
            g2 = sns.violinplot(x="time_type", y="time_val", hue="image",data=data_py[data_py.time_type == timings[i*3+j]], ax=ax_py[i][j])
            if j == 0:
